...
class ResCompany(models.Model):
    ...
    receivable_account_id = fields.Many2one('account.account', string='Cuenta Recibos IGTF', 
        domain=[('reconcile', '=', True), ('user_type_id.type', '=', 'receivable')], ondelete='restrict')
    payable_account_id = fields.Many2one('account.account', string='Cuenta Pagos IGTF', 
        domain=[('reconcile', '=', True), ('user_type_id.type', '=', 'payable')], ondelete='restrict')
...
class AccountPayment(models.Model):
    ...
    def _prepare_move_line_default_vals_igft(self, write_off_line_vals=None):
        ...
        if not self.igtf_journal_id.payment_debit_account_id or not self.igtf_journal_id.payment_credit_account_id:
            raise UserError(_(
                "You can't create a new payment without an outstanding payments/receipts account set on the %s journal.",
                self.igtf_journal_id.display_name))
